...
interface TemporalResultsDropdownProps {
  temporalResults: {
    ...
    effects: Record<string, Record<string, { baseline: number; value: number; absolute_change: number; percent_change: number }>>
  }
  onClear: () => void
}
...
function TemporalResultsDropdown({ temporalResults, onClear }: TemporalResultsDropdownProps) {
  ...
  const { indicators } = useSimulationStore()
  ...
  // Get final year effects sorted by magnitude
  const { affectedCount, rows } = useMemo(() => {
    ...
    const sorted = Object.entries(finalEffects)
      .map(([id, effect]) => {
        ...
        return {
          ...
          baseline: e.baseline,
          simulated: e.simulated ?? e.value,
          percentChange: e.percent_change
        }
      })
      .filter(row => Math.abs(row.percentChange) > 0.05)
      .sort((a, b) => Math.abs(b.percentChange) - Math.abs(a.percentChange))

    return { affectedCount: sorted.length, rows: sorted }
  }, [temporalResults, indicatorNames])
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          ...
          <span style={{ fontSize: 12, color: '#2E7D32' }}>
            {affectedCount} indicators affected
          </span>
